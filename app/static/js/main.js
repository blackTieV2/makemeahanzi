...
function setStrokeProgress(stroke, progress) {
    ...
    stroke.path.style.strokeDashoffset = `${stroke.length * (1 - clamped)}`;
}
...
function createStrokeSvg(strokes, strokeWidth) {
    ...
    strokes.forEach((data) => {
        ...
        path.setAttribute('stroke', '#d43f5f');
...
function resetAnimation(options = {}) {
    ...
    anim.strokes.forEach((stroke) => {
        stroke.path.classList.remove('workspace__stroke--drawn', 'workspace__stroke--active');
        stroke.path.setAttribute('stroke', '#d43f5f');
...
function setupStrokes(strokes) {
    ...
    const cssValue = parseFloat(rootStyles.getPropertyValue('--anim-stroke-width')) || 6;
    const viewerRect = strokeViewer.getBoundingClientRect();
    const svgSize = viewerRect.width || viewerRect.height || 512;
    const strokeWidth = (svgSize / 1024) * cssValue;

    const svg = createStrokeSvg(strokes, strokeWidth);
...
function animateSingleStroke(index, onDone) {
    ...
    path.classList.remove('workspace__stroke--drawn');
    path.setAttribute('stroke', '#d43f5f');
